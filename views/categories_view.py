...
class CategoriesView(QWidget):
    ...
    def _on_save(self) -> None:
        ...
        category_type = self._ui.cmb_type.currentText()
        color = self._ui.txt_color.text().strip() or None

        if self._editing_id is not None:
            self.update_requested.emit(self._editing_id, name, category_type, color or "")
        else:
            self.save_requested.emit(name, category_type, color or "")
    ...
    def load_categories(self, categories: List[Category]) -> None:
        ...
        for category in categories:
            ...
            self._ui.tbl_categories.setItem(row, 2, QTableWidgetItem(type_label))
            self._ui.tbl_categories.setItem(row, 3, QTableWidgetItem(category.color or ""))
            self._ui.tbl_categories.setItem(row, 4, QTableWidgetItem("Sim" if category.is_active else "Não"))
